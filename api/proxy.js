import {
  extractGitHubUrl, proxyFetch, handlePreflight, extractUrlFromPost,
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST' && !targetUrl) {
      const fakeReq = { headers: new Headers(), json: null, text: null };
      if (req.headers) {
      ...
      fakeReq.body = body;
      fakeReq.json = async () => JSON.parse(body);
      fakeReq.text = async () => body;
